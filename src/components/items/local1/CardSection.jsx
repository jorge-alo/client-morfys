                ...
                <section key={categoria} className="section-Card" id={categoria.replace(/\s+/g, '-')} >
                    ...
                    <div className="section-card__flex" >
                        {comidasDeCategoria.map((comida) => (
                            <Card key={comida.id} onClick={() => handleClickCard(comida)} id={comida.id} name={comida.name} description={comida.description} image={comida.image} price={comida.price} categoria={comida.categoria} tamanio= {comida.tamanio} controlunidad = {comida.controlUnidad} opciones= {comida.variantes && comida.variantes.length > 0 ? comida.variantes[0].opciones : []} ></Card>
                        ))}
                    </div>
                </section>
            ))
